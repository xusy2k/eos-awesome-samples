...
```sh
docker stop notechain_container
```
...
```sh
docker exec -it notechain_container bash
```
...
```sh
./scripts/deploy_contract.sh notechain notechai.acc notechainwal $(cat notechain_wallet_password.txt)
```
...
```sh
docker run --rm --name notechain_container \
